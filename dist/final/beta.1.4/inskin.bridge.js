                    ...
                    switch (data.method) {
                      ...
                      case "applyActionToLayer":
                        ...
                        if ("background-color" === data.payload.action) {
                            var _layer2 = $("#" + data.payload.id), layerLinksRaw = (_layer2.prop("tagName"), 
                            _layer2.attr("data-ism-links") && _layer2.attr("data-ism-links").split(",")), layerLinks = (layerLinksRaw || []).map(function(l) {
                                return l.trim();
                            });
                            supportedStylesheets.map(function(file) {
                        ...
                        if ("image" === data.payload.action) {
                            var _layer3 = $("#" + data.payload.id), _layerType2 = _layer3.prop("tagName"), _layerLinksRaw = _layer3.attr("data-ism-links") && _layer3.attr("data-ism-links").split(","), _layerLinks = (_layerLinksRaw || []).map(function(l) {
                                return l.trim();
                            });
                            if ("DIV" === _layerType2) {
                                supportedStylesheets.map(function(file) {
                                    data.payload.value ? changeStylesheetRule(file, "#" + data.payload.id, [ [ "background-image", "url(" + data.payload.value + ")" ] ]) && sendChangesToCore(_this.creativeId, file) : changeStylesheetRule(file, "#" + data.payload.id, [ [ "background-image", "" ] ]) && sendChangesToCore(_this.creativeId, file);
                                });
                        ...
                        if ("video-poster" === data.payload.action) {
                            var _layer4 = $("#" + data.payload.id + " #poster"), _layerType3 = _layer4.prop("tagName");
                            if (_layer4 && "IMG" === _layerType3) {
                                _layer4.attr("src", data.payload.value);
                                var _fileName2 = document.location.href.replace(/^.*[\\\/]/, "").split("?")[0];
                                sendPropsChangesToCore(_this.creativeId, _fileName2, [ {
                        ...
                        if ("video" === data.payload.action) {
                            ...
                            var _fileName3 = document.location.href.replace(/^.*[\\\/]/, "").split("?")[0], videoObjHandler = {};
                            videoObjHandler["#" + data.payload.id] = [ "data-ism-video-url", data.payload.value ];
                            sendPropsChangesToCore(_this.creativeId, _fileName3, [ videoObjHandler ]);
                        }
                        if ("video-auto-play" === data.payload.action) {
                            $("#" + data.payload.id).attr("data-ism-auto-play", data.payload.value);
                            var _fileName4 = document.location.href.replace(/^.*[\\\/]/, "").split("?")[0], _videoObjHandler = {};
                            _videoObjHandler["#" + data.payload.id] = [ "data-ism-auto-play", data.payload.value ];
                            sendPropsChangesToCore(_this.creativeId, _fileName4, [ _videoObjHandler ]);
                            askForReload();
                        }
                        break;
                    ...
                    if (layerObj.attr("data-ism-layer")) {
                        var fileName = document.location.href.replace(/^.*[\\\/]/, "").split("?")[0].replace(".html", ""), temp = {
                            ...
                            style: {
                                ...
                                width: layerObj.css("width") ? layerObj.css("width") : "0px",
                                height: layerObj.css("height") ? layerObj.css("height") : "0px"
                            }
                        };
                        all.push(temp);
                    }
